<template>
	<view>
		...
		<label>目的地位置<text class="subRed">*必填</text></label>
		<input placeholder="中国北京朝阳区/查看位置" v-model="distination" />
	...
	export default {
		...
		onShow() {
			...
			if(this.leaves.length > 0){
				...
				this.distination=leave.distination;
				this.cc = leave.cc;
		...
		methods: {
			sub: function() {
				...
				if (this.name == '' || this.type == '' || this.startDate == '' || this.startTime == '' ||
					...
					this.cc == '' || this.phone == '' || this.oneCheckTime == ''  ||
				...
				obj['distination'] = this.distination;
				obj['cc'] = this.cc;
